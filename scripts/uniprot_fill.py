...
def fetch_uniprot_batch(acc_list):
    ...
    return length_dict, records, taxonomy_dict
...
def main():
    ...
    accessions = df["uniprot_accession"].dropna().astype(str).tolist()
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="并发查询 UniProt"):
            batch_lengths, batch_records, batch_taxonomy = future.result()
            if batch_lengths:
    ...
    # Update df
    df["length"] = df["uniprot_accession"].map(length_map).fillna(df["length"])
    df["sequence"] = df["uniprot_accession"].map(sequence_map).fillna(df.get("sequence", ""))
    ...
    for col_name, tax_key in [("kingdom", "kingdom"), ("phylum", "phylum"), ("class", "class"), ("order", "order"), ("family", "family"), ("genus", "genus"), ("species", "species")]:
        tax_series = df["uniprot_accession"].map(
