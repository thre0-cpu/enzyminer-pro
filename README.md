...
打开 Property Prediction 页面只会检查缓存状态，不会自动启动预测。Recommendation 页面也不会隐式运行预测；如果尚无预测结果，仍可运行推荐，但 `predictedScore` 的贡献为 0。Compare 工作流同样将 Property Prediction 与 Recommendation 展示为两个独立步骤。
